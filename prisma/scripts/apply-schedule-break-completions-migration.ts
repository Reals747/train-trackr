/**
 * Idempotent applier for migration 20260618120000_schedule_break_completions.
 * See apply-dnd-migration.ts — `prisma migrate deploy/status` can hang on Supabase pgbouncer.
 *
 * Run: npx tsx prisma/scripts/apply-schedule-break-completions-migration.ts
 */
...
const MIGRATION_NAME = "20260618120000_schedule_break_completions";
...
async function main() {
  ...
  const before = (await prisma.$queryRawUnsafe(`
    SELECT
      EXISTS (
        ...
        WHERE table_schema = 'public' AND table_name = 'ScheduleBreakCompletion'
      ) AS has_table,
      EXISTS (
  ...
  await prisma.$transaction(async (tx) => {
    await tx.$executeRawUnsafe(`
      CREATE TABLE IF NOT EXISTS "ScheduleBreakCompletion" (
        "id" TEXT NOT NULL,
    ...
    await tx.$executeRawUnsafe(`
      CREATE INDEX IF NOT EXISTS "ScheduleBreakCompletion_storeId_profileKey_dateKey_idx"
      ON "ScheduleBreakCompletion"("storeId", "profileKey", "dateKey")
    `);

    await tx.$executeRawUnsafe(`
      CREATE UNIQUE INDEX IF NOT EXISTS "ScheduleBreakCompletion_storeId_profileKey_dateKey_employeeId_breakKey_key"
      ON "ScheduleBreakCompletion"("storeId", "profileKey", "dateKey", "employeeId", "breakKey")
    `);

    await tx.$executeRawUnsafe(`
      DO $$ BEGIN
        ALTER TABLE "ScheduleBreakCompletion"
          ADD CONSTRAINT "ScheduleBreakCompletion_storeId_fkey"
  ...
  if (!after[0]?.has_table) {
    throw new Error("Migration verification failed — ScheduleBreakCompletion table missing.");
  }
